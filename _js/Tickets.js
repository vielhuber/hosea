    ...
    static deleteTicket(ticket_id) {
        return new Promise((resolve, reject) => {
            Store.data.api
                ...
                .then(response => {
                    Store.data.tickets.forEach(
                        (tickets__value, tickets__key) => {
                            if (tickets__value.id == ticket_id) {
            ...
            if (event.ctrlKey || event.metaKey) {
                if (String.fromCharCode(event.which).toLowerCase() === 'd') {
                    ...
                    Tickets.createTicket(
                        Tickets.getTicketData(current.getAttribute('data-id'))
                    )
                        .then(ticket => {
                            current.insertAdjacentHTML(
                                'afterend',
                                Html.createHtmlLine(ticket, true)
                            );
                            current.nextElementSibling
                                .querySelector(
                                    'td:nth-child(' + currentIndex + ')'
                                )
        ...
        document.querySelector('.tickets').addEventListener('change', e => {
            if (e.target.closest('.tickets__entry [name="date"]')) {
                if (e.target.value != '') {
                    ...
                    ticket_dates.forEach(
                        (ticket_dates__value, ticket_dates__key) => {
    ...
    static updateSum() {
        ...
        Store.data.tickets.forEach(tickets__value => {
            if (
                ...
                tickets__value.time != '' &&
                !['idle', 'done', 'billed', 'delegated'].includes(
